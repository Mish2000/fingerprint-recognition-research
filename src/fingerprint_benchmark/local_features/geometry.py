...
from .types import LocalFeatureRepresentation


class _GeometryConfig(Protocol):
    minimum_geometry_matches: int
    normalize_coordinates_by_ppi: bool
    reference_ppi: float
    rng_seed: int
...
def verify_geometry(
    representation_a: LocalFeatureRepresentation,
    representation_b: LocalFeatureRepresentation,
...
def _failure(reason: str, submitted: int, config: _GeometryConfig) -> GeometryResult:
    return GeometryResult(
        success=False,
...
__all__ = ["GeometryResult", "verify_geometry"]
