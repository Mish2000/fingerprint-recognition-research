"""Build and validate the SD300c plain_roll genuine-pair manifest."""
...
from .plain_roll_protocol import (
    PROTOCOL,
    ManifestValidationReport,
    PlainRollPair,
    build_plain_roll_pairs as build_context_plain_roll_pairs,
...
class ManifestGenerationError(ValueError):
    """Raised when the SD300c plain_roll manifest cannot be generated safely."""


class ManifestValidationError(ValueError):
    """Raised when a plain_roll manifest fails validation."""
...
def build_plain_roll_pairs(scan_result: ScanResult) -> list[PlainRollPair]:
    """Build deterministic genuine SD300c plain-roll pairs from a completed SD300c scan."""

    return build_context_plain_roll_pairs(DATASET_CONTEXT, scan_result, ManifestGenerationError)
...
def generate_manifest(
    ...
    """Scan SD300c, validate a candidate manifest, then atomically replace the target."""

    scan_result = scan_dataset(data_root, DATASET_CONTEXT.spec)
    pairs = build_plain_roll_pairs(scan_result)
...
def write_manifest_atomic(pairs: Iterable[PlainRollPair], output_path: Path) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate or validate the SD300c plain_roll manifest."
    )
