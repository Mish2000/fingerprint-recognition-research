"""Build and validate the SD300c roll_self protocol manifest."""
...
class ManifestGenerationError(ValueError):
    """Raised when the SD300c roll_self manifest cannot be generated safely."""
...
def build_roll_self_pairs(scan_result: ScanResult) -> list[RollSelfPair]:
    """Build deterministic SD300c roll_self pairs from a completed SD300c scan."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate or validate the SD300c roll_self manifest."
    )
