"""Context-aware implementation of the roll_self protocol."""
...
def build_roll_self_pairs(
    context: ProtocolDatasetContext,
    scan_result: ScanResult,
    error_cls: type[Exception],
) -> list[RollSelfPair]:
    """Build deterministic roll_self pairs from a completed dataset scan."""
    ...
    for record in scan_result.records:
        if record.impression_type != "roll":
            continue
...
def validate_manifest(
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        _ensure_path_under_roll_dataset(context, path, expected_roll_dir, row_number, validation_error_cls)
        source_record = _validate_source_path(context, path, row_number, validation_error_cls)
        _validate_row_matches_record(context, row, source_record, ppi, raw_frgp, row_number, validation_error_cls)
...
def _identity(pair: RollSelfPair) -> tuple[str, str, int]:
...
def _expected_pairs_by_identity(
    ...
    generation_error_cls: type[Exception],
    validation_error_cls: type[Exception],
) -> dict[tuple[str, str, int], RollSelfPair]:
    scan_result = scan_dataset(data_root, context.spec)
    try:
        pairs = build_roll_self_pairs(context, scan_result, generation_error_cls)
...
def _canonical_position_for_row(
    ...
    try:
        return canonical_finger_position("roll", raw_frgp)
    except CanonicalFingerMappingError as exc:
        raise validation_error_cls(
            f"raw FRGP is not valid for roll at CSV line {row_number}: {exc}"
        ) from exc
...
def _format_duplicate_identities(
    ...
    return "Duplicate roll anatomical identities found: " + "; ".join(details)
...
def _ensure_path_under_roll_dataset(
    ...
    try:
        ...
        raise validation_error_cls(
            f"Path at CSV line {row_number} is not under the {_dataset_label(context)} roll directory: {path}"
        ) from exc
...
def _validate_source_path(
    ...
    try:
        return validate_image_path(path, context.spec, "roll")
    except SchemaValidationError as exc:
        raise validation_error_cls(
            f"Path does not validate as an {_dataset_label(context)} roll record at CSV line "
            f"{row_number}: {exc}"
        ) from exc


def _validate_row_matches_record(
    context: ProtocolDatasetContext,
    row: dict[str, str],
    record: ImageRecord,
    ppi: int,
    ...
    if record.dataset != context.name:
        raise validation_error_cls(f"Path dataset mismatch at CSV line {row_number}.")
    if record.impression_type != "roll":
        raise validation_error_cls(f"Path is not a roll record at CSV line {row_number}.")
    if record.subject_id != row["subject_id"]:
        raise validation_error_cls(
            f"Subject mismatch at CSV line {row_number}: manifest has {row['subject_id']!r}, "
            f"path has {record.subject_id!r}."
        )
    if record.ppi != ppi:
        raise validation_error_cls(
            f"PPI mismatch at CSV line {row_number}: manifest has {ppi}, path has {record.ppi}."
        )
    if record.ppi != context.expected_ppi:
        raise validation_error_cls(
            f"PPI mismatch at CSV line {row_number}: path has {record.ppi}, "
            f"{context.name} expects {context.expected_ppi}."
        )
    if record.frgp != raw_frgp:
        raise validation_error_cls(
            f"FRGP mismatch at CSV line {row_number}: manifest has {raw_frgp}, path has {record.frgp}."
        )


def _validate_expected_source_records(
    context: ProtocolDatasetContext,
    expected_pairs: dict[tuple[str, str, int], RollSelfPair],
    actual_rows: dict[tuple[str, str, int], _ManifestRowRecord],
    validation_error_cls: type[Exception],
) -> None:
