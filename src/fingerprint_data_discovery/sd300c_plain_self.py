"""Build and validate the SD300c plain_self protocol manifest."""
...
from .plain_self_protocol import (
    PROTOCOL,
    ManifestValidationReport,
    PlainSelfPair,
    build_plain_self_pairs as build_context_plain_self_pairs,
...
class ManifestGenerationError(ValueError):
    """Raised when the SD300c plain_self manifest cannot be generated safely."""


class ManifestValidationError(ValueError):
    """Raised when a plain_self manifest fails validation."""
...
def build_plain_self_pairs(scan_result: ScanResult) -> list[PlainSelfPair]:
    """Build deterministic SD300c plain_self pairs from a completed SD300c scan."""

    return build_context_plain_self_pairs(DATASET_CONTEXT, scan_result, ManifestGenerationError)
...
def generate_manifest(
    ...
    pairs = build_plain_self_pairs(scan_result)
...
def write_manifest_atomic(pairs: Iterable[PlainSelfPair], output_path: Path) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate or validate the SD300c plain_self manifest."
    )
