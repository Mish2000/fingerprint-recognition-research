"""Cross-protocol audit for the existing SD300b manifests."""
...
from . import sd300b_plain_roll
from . import sd300b_plain_self
from . import sd300b_roll_self
...
class ProtocolAuditError(ValueError):
    """Raised when the SD300b protocol cross-audit fails."""
...
def build_protocol_audit_report(
    ...
    plain_self_manifest: Path = sd300b_plain_self.DEFAULT_MANIFEST_PATH,
    roll_self_manifest: Path = sd300b_roll_self.DEFAULT_MANIFEST_PATH,
    plain_roll_manifest: Path = sd300b_plain_roll.DEFAULT_MANIFEST_PATH,
) -> ProtocolAuditReport:
    """Validate the three SD300b manifests, then check their cross-protocol relation."""
    ...
    plain_self_records = _read_self_records(
        plain_self_manifest,
        expected_protocol=sd300b_plain_self.PROTOCOL,
        raw_frgp_column="raw_frgp_a",
        path_column="path_a",
    )
    roll_self_records = _read_self_records(
        roll_self_manifest,
        expected_protocol=sd300b_roll_self.PROTOCOL,
        raw_frgp_column="raw_frgp_a",
        path_column="path_a",
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Audit consistency across the existing SD300b protocol manifests."
    )
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        "--plain-self-manifest",
        type=Path,
        default=sd300b_plain_self.DEFAULT_MANIFEST_PATH,
        help=f"plain_self manifest path. Defaults to {sd300b_plain_self.DEFAULT_MANIFEST_PATH}.",
    )
    parser.add_argument(
        "--roll-self-manifest",
        type=Path,
        default=sd300b_roll_self.DEFAULT_MANIFEST_PATH,
        help=f"roll_self manifest path. Defaults to {sd300b_roll_self.DEFAULT_MANIFEST_PATH}.",
    )
    parser.add_argument(
        "--plain-roll-manifest",
        type=Path,
        default=sd300b_plain_roll.DEFAULT_MANIFEST_PATH,
        help=f"plain_roll manifest path. Defaults to {sd300b_plain_roll.DEFAULT_MANIFEST_PATH}.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def _run_dedicated_validators(
    ...
    validators: list[tuple[str, Callable[[Path, Path], object], Path]] = [
        ("plain_self", sd300b_plain_self.validate_manifest, plain_self_manifest),
        ("roll_self", sd300b_roll_self.validate_manifest, roll_self_manifest),
        ("plain_roll", sd300b_plain_roll.validate_manifest, plain_roll_manifest),
    ]
...
def _read_plain_roll_records(manifest_path: Path) -> dict[Identity, _PlainRollRecord]:
    ...
    for row_number, row in enumerate(rows, start=2):
        validate_required_values(row, row_number, ProtocolAuditError)
        _validate_constant(row, "dataset", DATASET, row_number)
        _validate_constant(row, "protocol", sd300b_plain_roll.PROTOCOL, row_number)
        identity = _identity_from_row(row, row_number)
        if identity in records:
            raise ProtocolAuditError(
